...
# Page config
st.set_page_config(
    page_title="Product Requirements Assistant", page_icon="📋", layout="wide"
)
...
def handle_api_error(error_msg: str, operation: str = "operation"):
    ...
    if "Could not connect" in error_msg:
        st.error(
            "❌ Cannot connect to backend. Please check that the backend server is running."
        )
...
def main():
    ...
    try:
        ...
        st.error(
            "❌ Cannot connect to backend. Please ensure the application is running properly."
        )
    ...
    with st.sidebar:
        ...
        st.markdown(
            '<div style="margin-top: 1rem;"><p style="font-size: 0.7rem; font-weight: 600; '
            "letter-spacing: 0.05em; margin-bottom: 0.5rem; color: #888; "
            'text-transform: uppercase;">Recent Projects</p></div>',
            unsafe_allow_html=True,
        )
        ...
        if not projects:
            ...
            if st.session_state.current_project:
                try:
                    current_idx = next(
                        ...
                        if p["id"] == st.session_state.current_project["id"]
                    )
            ...
            if (
                ...
                or st.session_state.current_project["id"] != selected_project["id"]
            ):
...
def show_new_project_form():
    ...
    with st.form("new_project"):
        title = st.text_input(
            "Document Title", placeholder="e.g., AI Chat Widget Integration"
        )
...
def show_phase_1():
    ...
    st.info(
        "1. Select all text in the box above (Ctrl/Cmd+A)\\n"
        "2. Copy it (Ctrl/Cmd+C)\\n"
        "3. Paste into Claude Sonnet 4.5 (at claude.ai)\\n"
        "4. Engage in dialogue to refine the PRD\\n"
        "5. **IMPORTANT**: In Claude, use the 'Copy' button at the bottom of Claude's response "
        "to copy the raw markdown\\n"
    ...
    with col1:
        ...
        claude_response = st.text_area(
            ...
            height=600,
            key="phase1_content",
            help="Make sure to use Claude's 'Copy' button at the bottom of the response to copy "
            "raw markdown with formatting preserved",
        )

        # Check for HTML tags that indicate wrong copy method
        if claude_response and (
            "<input" in claude_response
            or "<checkbox" in claude_response
            or "<div" in claude_response
        ):
            st.error(
                "⚠️ HTML tags detected! You've copied rendered HTML instead of markdown. "
                "Please use Claude's Copy button to copy the raw markdown."
            )
        ...
        if claude_response:
            with st.container():
                ...
                if "<input" in cleaned_content:
                    st.warning(
                        "Note: HTML elements detected and won't render properly. "
                        "Use Claude's Copy button next time."
                    )
        ...
        if st.button("💾 Save Phase 1", type="primary"):
            if claude_response:
                with st.spinner("Saving Phase 1 and generating Phase 2 prompt..."):
                    try:
                        updated_project = api.update_phase(
                            project["id"], 1, claude_response
                        )
...
def show_phase_2():
    ...
    with col1:
        ...
        if gemini_response and (
            "<input" in gemini_response
            or "<checkbox" in gemini_response
            or "<div" in gemini_response
        ):
            st.error(
                "⚠️ HTML tags detected! Please copy the markdown source, not the rendered HTML."
            )
    ...
    with col2:
        ...
        if gemini_response:
            with st.container():
                ...
                if "<input" in cleaned_content:
                    st.warning(
                        "Note: HTML elements detected. Copy markdown source next time."
                    )
                    cleaned_content = re.sub(r"<input[^>]*>", "- [ ]", cleaned_content)
                st.markdown(cleaned_content)
        else:
...
def show_phase_3():
    ...
    with col1:
        ...
        # Check for HTML tags
        if final_prd and (
            "<input" in final_prd or "<checkbox" in final_prd or "<div" in final_prd
        ):
            st.error(
                "⚠️ HTML tags detected! Please use Claude's Copy button to copy raw markdown."
            )
    ...
    with col2:
        ...
        if final_prd:
            with st.container():
                ...
                if "<input" in cleaned_content:
                    st.warning(
                        "Note: HTML elements detected. Use Claude's Copy button next time."
                    )
                    cleaned_content = re.sub(r"<input[^>]*>", "- [ ]", cleaned_content)
                st.markdown(cleaned_content)
        else:
    ...
    with col1:
        if st.button("💾 Save Final PRD", type="primary"):
            if final_prd:
                with st.spinner(
                    "Saving Final PRD and generating comprehensive document..."
                ):
                    try:
    ...
    with col4:
        if phase_data.get("content"):
            ...
            final_doc += (
                "3. [Phase 1: Initial Claude PRD](#phase-1-initial-claude-prd)\\n"
            )
            ...
            filename_complete = (
                f"{project['title'].replace(' ', '_')}_COMPLETE_{timestamp}.md"
            )
            st.download_button(
