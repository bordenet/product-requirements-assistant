...
def main():
    ...
    try:
        ...
        st.error("❌ Cannot connect to backend. Please ensure the application is running properly.")
    ...
    with st.sidebar:
        ...
        if not projects:
            ...
            if st.session_state.current_project:
                try:
                    current_idx = next(i for i, p in enumerate(projects)
            ...
            if (st.session_state.current_project is None or
                    st.session_state.current_project['id'] != selected_project['id']):
...
def show_phase_1():
    ...
    with col1:
        ...
        if claude_response and (
                '<input' in claude_response or '<checkbox' in claude_response or
                '<div' in claude_response):
            st.error(
...
def show_phase_2():
    ...
    with col1:
        ...
        if gemini_response and (
                '<input' in gemini_response or '<checkbox' in gemini_response or '<div' in gemini_response):
