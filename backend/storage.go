...
func loadPrompt(phase string) (string, error) {
	...
	// Try loading .md file first (new format)
	mdFilename := filepath.Join(getPromptsDir(), fmt.Sprintf("%s.md", phase))
	data, err := fm.ReadFile(mdFilename)
	if err == nil {
		return string(data), nil
	}

	// Fall back to .txt file (legacy format)
	txtFilename := filepath.Join(getPromptsDir(), fmt.Sprintf("%s.txt", phase))
	data, err = fm.ReadFile(txtFilename)
	if err != nil {
...
func getDefaultPrompt(phase string) string {
	switch phase {
	case "claude_initial", "phase1-claude-initial":
...
Here are a bunch of simplifications, considerations, and other associated context you need to be aware of: %s`
	...
	case "gemini_review", "phase2-gemini-review":
		return `Forget our previous sessions-- start fresh with me. You are a principal-level Product Manager at a technology company. You don't know how to code, and I don't want you to know how to code. Again, don't provide any code or JSON schema or SQL queries, ever. You will help me review a product requirements document (PRD), attached, generated by Claude Sonnet 4.5 and help distill it / simplify it / tease critical details out so that we hand a more mature, less ambiguous document to the engineering team.
...
[PASTE CLAUDE'S ORIGINAL PRD HERE]`
	...
	case "claude_compare", "phase3-claude-synthesis":
